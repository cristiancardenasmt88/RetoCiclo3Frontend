function autoInicioRelacionCliente(){
    ...
    $.ajax({
        url:"http://144.22.57.193:8080/api/Client/all",
...
function agregarReservation() {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            url:"http://144.22.57.193:8080/api/Reservation/save",
...
function listarReservation(){
    ...
    $.ajax({
        url:"http://144.22.57.193:8080/api/Reservation/all",
...
function pintarRespuestaReservation(response){
    ...
    myTable+="<tr>";
        myTable+="<td>"+cnvFecha(response[i].startDate)+"</td>";
        myTable+="<td>"+cnvFecha(response[i].devolutionDate)+"</td>";
        ...
        myTable+='<td><button  onclick="cargarDatosReservation(' + response[i].idReservation + ')">Editar</button></td>';
    ...
    $.ajax(
        {
            dataType: 'json',
            data: dataToSend,
            url:"http://144.22.57.193:8080/api/Reservation/" + idElemento,
...
function cargarDatosReservation(id) {
    $.ajax({
        dataType: 'json',
        url:"http://144.22.57.193:8080/api/Reservation/"+id,
        ...
        success: function (response) {
            ...
            $("#startDate").val(cnvFecha(item.startDate));
            $("#devolutionDate").val(cnvFecha(item.devolutionDate));
...
function actualizarReservation(idElemento) {
    ...
    if($("#startDate").val().length == 0 || $("#devolutionDate").val().length == 0 || $("#status").val().length == 0){
        ...
        $.ajax({
            ...
            data: dataToSend,
            contentType: "application/JSON",
            url:"http://144.22.57.193:8080/api/Reservation/update",
